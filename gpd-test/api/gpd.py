...
from models.dto import FileSendResult, SendStatus
...
if not EVENTHUB_TOPIC:
    raise RuntimeError(
        "EVENTHUB_TOPIC is not configured. Please set 'eventhub_topic' in your settings or the EVENTHUB_TOPIC env var."
    )


def require_producer_service(request: Request, *, missing_status: int = 503) -> ProducerService:
    svc: ProducerService | None = getattr(request.app.state, "producer_service", None)
    if svc is None:
        raise HTTPException(
            status_code=missing_status, detail={"status": "error", "message": "Producer service is not available"}
        )
    return svc
