...
def test_create_debt_position(gpd_test_data, environment):
    ...
    payload = generate_debt_position_create_payload(
        debtor_fc=gpd_test_data.debtor_fc, iupd=gpd_test_data.iupd, iuv=gpd_test_data.iuv
    )

    res = environment["create_function"](
        gpd_test_data.subscription_key, gpd_test_data.organization_id, payload, to_publish=True
    )
...
def test_delete_debt_position(gpd_test_data, environment):
    ...
    delete_response = environment["delete_function"](
        gpd_test_data.subscription_key, gpd_test_data.organization_id, gpd_test_data.iupd
    )
...
def test_update_debt_position(gpd_test_data, environment):
    ...
    create_response = environment["create_function"](
        gpd_test_data.subscription_key, gpd_test_data.organization_id, create_payload, to_publish=True
    )
    assert create_response.status_code == 201, f"Expected 201 but got {create_response.status_code}"
    ...
    update_payload = generate_debt_position_update_payload(
        iupd=gpd_test_data.iupd, debtor_fc=gpd_test_data.debtor_fc, original_iuv=gpd_test_data.iuv
    )
