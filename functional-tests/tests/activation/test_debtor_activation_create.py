...
@allure.feature("Activation")
@allure.story("Debtor activation")
@allure.title("A second service provider triggers takeover when debtor is already active on another service provider")
...
def test_activate_debtor_already_active_on_another_service_provider_triggers_takeover(
    ...
    assert res_a.status_code == 201, f"Error activating debtor, expected 201 but got {res_a.status_code} - {res_a.text}"

    res_b = activate_with_sp_b(debtor_service_provider_token_b, random_fiscal_code)
    assert res_b.status_code == 409, f"Expected 409 conflict (takeover trigger), got {res_b.status_code} - {res_b.text}"
...
@allure.feature("Activation")
@allure.story("Debtor activation")
@allure.title("A debtor cannot be activated more than once by the same service provider")
