...
def test_get_all_activations(debtor_service_provider_token_a, next_cursor):
    ...
    body = res.json()
    assert isinstance(
        body.get('activations'), list
    ), "Expected 'activations' to be a list"

    page_meta = body.get('page') or body.get('metadata')
    if page_meta is not None:
        assert isinstance(
            page_meta, dict
        ), 'Expected metadata to be a dict when present'

    nid = next_cursor(res)
    if nid:
        res2 = get_all_activations(
            debtor_service_provider_token_a, size=page_size, next_activation_id=nid
        )
        assert res2.status_code == 200, f"Expected 200 but got {res2.status_code}"
        body2 = res2.json()
        assert isinstance(
            body2.get('activations'), list
        ), "Expected 'activations' to be a list"
        assert (
            len(body2['activations']) <= page_size
        ), f"Expected {page_size} or fewer activations in paginated response"
...
@allure.title(
    'A debtor service provider fails activation due to wrong service provider id'
)
...
def test_get_all_activations_invalid_params(debtor_service_provider_token_a, size):
    ...
    assert (
        res.status_code == 400
    ), f"Expected 400 for invalid params, got {res.status_code}"
    body = res.json()
...
def test_get_all_activations_unauthorized():
    ...
    assert (
        body['statusCode'] == 401
    ), f"Expected statusCode 401 but got {body['statusCode']}"
...
def test_get_all_activations_nonexistent_next_activation_id(
    ...
    res = get_all_activations(
        debtor_service_provider_token_a, size=5, next_activation_id=random_cursor
    )

    assert res.status_code == 200, f"Expected 200 but got {res.status_code}"
    assert is_empty_response(
        res
    ), 'Expected empty body for nonexistent NextActivationId'
