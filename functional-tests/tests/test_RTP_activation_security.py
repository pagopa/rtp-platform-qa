...
def test_sp_b_cannot_get_activation_by_fiscal_code(make_activation, debtor_service_provider_token_b):
    _, debtor_fc = make_activation()

    response = get_activation_by_payer_id(access_token=debtor_service_provider_token_b, payer_fiscal_code=debtor_fc)
...
@pytest.mark.activation
@pytest.mark.unhappy_path
def test_sp_b_cannot_get_activation_by_id(make_activation, debtor_service_provider_token_b):

    activation_id, _ = make_activation()

    response = get_activation_by_id(access_token=debtor_service_provider_token_b, activation_id=activation_id)
    assert response.status_code == 404, (
...
def test_read_all_can_get_activation_by_fiscal_code(make_activation, sp_activations_read_all_token):
    _, debtor_fc = make_activation()

    response = get_activation_by_payer_id(access_token=sp_activations_read_all_token, payer_fiscal_code=debtor_fc)
...
@pytest.mark.activation
@pytest.mark.happy_path
def test_read_all_can_get_activation_by_id(make_activation, sp_activations_read_all_token):
    activation_id, _ = make_activation()

    response = get_activation_by_id(access_token=sp_activations_read_all_token, activation_id=activation_id)
    assert response.status_code == 200, (
