...
from utils.dataset_callback_data_DS_12N_negative_v2 import (
    generate_callback_data_DS_12N_negative_compliant,
    generate_non_compliant_callback_data_DS_12N_negative,
)
...
@allure.story("Service provider sends a v2 RFC callback with RJCR status")
@allure.title("A v2 RFC callback DS12N RJCR is successfully received and RTP status is CANCELLED_REJECTED")
@allure.tag("functional", "happy_path", "rtp_callback", "v2", "ds_12n_negative_compliant", "rfc")
@pytest.mark.callback
@pytest.mark.happy_path
def test_receive_rfc_callback_DS_12N_negative_compliant(
    ...
    """
    Test v2 RFC callback DS12N with RJCR status.
    ...
    6. Verify RTP status is CANCELLED_REJECTED
    """
...
@allure.tag("functional", "unhappy_path", "rtp_callback", "v2", "ds_12n_negative_compliant", "rfc")
@pytest.mark.callback
...
def test_fail_send_rfc_callback_wrong_certificate_serial_DS_12N_negative_compliant(
    ...
    """
    Test v2 RFC callback DS12N with wrong certificate identity.

    Flow:
    1. Activate payer
    2. Send an RTP via GPD message v2 (CREATE VALID)
    3. Cancel the RTP via GPD message v2 (DELETE) -> RTP to RFC_SENT
    4. Send DS12N callback with assignee_bic='FAKESP01' (doesn't match certificate identity)
...
@allure.feature("RTP Callback DS_12N - Negative")
@allure.story("Service provider sends a v2 RFC callback with RJCR status")
@allure.title("Failed v2 RFC callback for non existing Service Provider - DS-12N negative compliant")
@allure.tag("functional", "unhappy_path", "rtp_callback", "v2", "ds_12n_negative_compliant", "rfc")
@pytest.mark.callback
@pytest.mark.unhappy_path
def test_fail_send_rfc_callback_non_existing_service_provider_DS_12N_negative_compliant(
    ...
    """
    Test v2 RFC callback DS12N with non-existing service provider.
...
@allure.title("A v2 RFC callback DS12N INVALID is rejected with 400")
@allure.tag("functional", "unhappy_path", "rtp_callback", "v2", "ds_12n_invalid", "rfc")
@pytest.mark.callback
@pytest.mark.unhappy_path
def test_receive_rfc_callback_DS_12N_negative_invalid(
    rtp_consumer_access_token,
    debtor_service_provider_token_c,
    rtp_reader_access_token,
    debtor_sp_mock_cert_key,
    ...
    """
    Test v2 RFC callback DS12N with INVALID status.
...
@allure.feature("RTP Callback DS_12N - Negative")
@allure.story("Service provider sends a v2 RFC callback with RJCR status")
@allure.title("Failed v2 RFC callback for invalid RTP transition - DS-12N negative compliant")
@allure.tag("functional", "unhappy_path", "rtp_callback", "v2", "ds_12n_negative_compliant", "rfc")
@pytest.mark.callback
@pytest.mark.unhappy_path
def test_fail_send_rfc_callback_invalid_transition_DS_12N_negative_compliant(
    rtp_consumer_access_token,
    debtor_service_provider_token_c,
    rtp_reader_access_token,
    debtor_sp_mock_cert_key,
    ...
    """
    Test v2 RFC callback DS12N with RJCR status applied twice on the same RTP.

    Flow:
    1. Activate payer
    2. Send an RTP via GPD message v2 (CREATE VALID)
    3. Cancel the RTP via GPD message v2 (DELETE) -> RTP to RFC_SENT
    4. Send DS12N callback with CxlStsId RJCR (Rejected Cancellation Request) -> RTP to CANCELLED_REJECTED
    ...
    7. Verify RTP status remains CANCELLED_REJECTED
    """
    message_payload = generate_gpd_message_payload(fiscal_code=random_fiscal_code, operation="CREATE", status="VALID")
    ...
    first_callback_response = srtp_rfc_callback_v2(
        rtp_payload=callback_data,
        cert_path=certificate,
        key_path=key,
        include_version_header=False,
    ...
    second_callback_response = srtp_rfc_callback_v2(
        rtp_payload=callback_data,
        cert_path=certificate,
        key_path=key,
        include_version_header=False,
