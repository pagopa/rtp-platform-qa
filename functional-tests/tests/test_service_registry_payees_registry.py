...
def test_get_payees_success(pagopa_payee_registry_token):
    ...
    assert page_metadata["totalElements"] >= len(data["payees"]), (
        "Total elements should be at least the number of payees returned"
    )
    ...
    assert page_metadata["totalElements"] <= page_metadata["totalPages"] * page_metadata["size"], (
        "Total elements should not exceed the capacity of all pages"
    )
