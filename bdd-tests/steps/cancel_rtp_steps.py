...
def then_rtp_already_cancelled(context):
    assert context.cancel_rtp_response.status_code == 422, (
        f"Error cancelling RTP, got status code: {context.cancel_rtp_response.status_code}"
    )


@then("the RTP is cancelled")
def then_rtp_is_cancelled(context):
    assert context.cancel_rtp_response.status_code == 204, (
        f"Expected RTP to be cancelled, got status code: {context.cancel_rtp_response.status_code}"
    )
...
def then_rtp_is_not_found(context):
    assert context.latest_cancel_response.status_code == 404, (
        f"Expected to not find the RTP, got status code: {context.latest_cancel_response.status_code}"
    )
