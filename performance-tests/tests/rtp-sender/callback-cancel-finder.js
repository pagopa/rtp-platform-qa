...
/** Scenario max duration (`TEST_MAX_DURATION`, default: 240m). @type {string} */
const TEST_MAX_DURATION = __ENV.TEST_MAX_DURATION || '240m';

/** Scenario graceful stop (`TEST_GRACEFUL_STOP`, default: 30s). @type {string} */
const TEST_GRACEFUL_STOP = __ENV.TEST_GRACEFUL_STOP || '30s';
...
/**
* Candidate input files for the cancel dataset.
* The default source is `resource-id.json` generated by GPD flows.
* Fallback keeps backward compatibility with the legacy cancel dataset.
*
* Supported env vars:
* - CANCEL_RESOURCE_IDS_FILE
* - RESOURCE_IDS_FILE
*
* @type {string[]}
*/
const resourceIdCandidates = [
   __ENV.CANCEL_RESOURCE_IDS_FILE,
   __ENV.RESOURCE_IDS_FILE,
...
for (const candidate of resourceIdCandidates) {
   try {
       const parsed = JSON.parse(open(candidate));
       const ids = Array.isArray(parsed) ? parsed : [parsed];
       resourceIds = Array.from(new Set(ids.map(String))).filter(Boolean);
       if (resourceIds.length > 0) {
           console.log(`📥 Loaded ${resourceIds.length} resource IDs from ${candidate}`);
           break;
       }
   } catch (error) {
       const message = error && error.message ? error.message : String(error);
       console.warn(`⚠️ Unable to load resource IDs from ${candidate}: ${message}`);
   }
}

if (resourceIds.length === 0) {
   throw new Error(`No resource IDs found in any configured cancel input file: ${resourceIdCandidates.join(', ')}`);
}
